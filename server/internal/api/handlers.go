...
func (r *Router) handleAgentWebSocket(c *gin.Context) {
	...
	// A hidden device that successfully authenticates is restored to the device
	// list, and the restore is surfaced as an alert + audit entry rather than
	// happening silently. No-op for devices that were not hidden.
	autoUnhideOnReconnect(ctx, r.db.Pool(), r.hub, deviceID, unhideTriggerWSAuth)
