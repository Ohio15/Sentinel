...
func NewRouter(cfg *config.Config, db *database.DB, cache *cache.Cache, hub *websocket.Hub) *gin.Engine {
	...
	api := r.Group("/api")
	{
		...
		protected.Use(middleware.CSRFMiddleware(csrfConfig))
		{
			...
			protected.POST("/devices/:id/generate-kill-token", middleware.RequireRole("admin"), router.generateKillTokenForDevice)
			protected.GET("/devices/:id/emergency-uninstall-script", middleware.RequireRole("admin"), router.getEmergencyUninstallScript)
			...
			protected.POST("/devices/:id/hide", middleware.RequireRole("admin", "operator"), router.hideDevice)
			protected.POST("/devices/:id/unhide", middleware.RequireRole("admin", "operator"), router.unhideDevice)
...
func NewRouterWithServices(services *Services) *gin.Engine {
	...
	api := r.Group("/api")
	{
		...
		protected.Use(middleware.CSRFMiddleware(middleware.DefaultCSRFConfig()))
		{
			...
			protected.POST("/devices/:id/generate-kill-token", middleware.RequireRole("admin"), generateKillTokenForDeviceHandler(services))
			protected.GET("/devices/:id/emergency-uninstall-script", middleware.RequireRole("admin"), getEmergencyUninstallScriptHandler(services))
			...
			protected.POST("/devices/:id/hide", middleware.RequireRole("admin", "operator"), hideDeviceHandler(services))
			protected.POST("/devices/:id/unhide", middleware.RequireRole("admin", "operator"), unhideDeviceHandler(services))
			...
			// Router Network (APM) - Scheduled Actions
			protected.GET("/router/scheduled-actions", listScheduledActionsHandler(services))
			protected.POST("/router/scheduled-actions", middleware.RequireRole("admin", "operator"), createScheduledActionHandler(services))
			protected.PUT("/router/scheduled-actions/:id", middleware.RequireRole("admin", "operator"), updateScheduledActionHandler(services))
			protected.DELETE("/router/scheduled-actions/:id", middleware.RequireRole("admin"), deleteScheduledActionHandler(services))
			protected.POST("/router/scheduled-actions/:id/toggle", middleware.RequireRole("admin", "operator"), toggleScheduledActionHandler(services))

			// Router Network (APM) - Write actions
			protected.POST("/router/block-device", middleware.RequireRole("admin", "operator"), handleBlockDevice(services))
			protected.POST("/router/allow-device", middleware.RequireRole("admin", "operator"), handleAllowDevice(services))
			protected.POST("/router/mark-known", middleware.RequireRole("admin", "operator"), handleMarkDeviceKnown(services))
			protected.POST("/router/send-wol", middleware.RequireRole("admin", "operator"), handleSendWOL(services))
			protected.POST("/router/speed-test", middleware.RequireRole("admin", "operator"), handleRunSpeedTest(services))
			protected.POST("/router/dismiss-anomaly", middleware.RequireRole("admin", "operator"), handleDismissAnomaly(services))
...
func rateLimitMiddleware(cache *cache.Cache, maxRequests int, windowSeconds int, prefix ...string) gin.HandlerFunc {
	...
	return func(c *gin.Context) {
		...
		// Set TTL when the counter is first created (count == 1) for a fixed-window rate limit.
		// Also set TTL if the key somehow has no expiry (immortal key bug: if the first
		// Expire call fails due to a Redis hiccup, the key lives forever causing permanent 429s).
		if count == 1 {
			cache.Expire(c.Request.Context(), key, windowSeconds)
		} else if ttl, err := cache.Client().TTL(c.Request.Context(), key).Result(); err == nil && ttl < 0 {
			cache.Expire(c.Request.Context(), key, windowSeconds)
		}
