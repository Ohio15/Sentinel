...
if errorlevel 1 (
    ...
    sc create SentinelAgent binPath= "\"C:\Program Files\Sentinel Agent\sentinel-agent.exe\" --server=http://localhost:8081 --token=REDACTED_ENROLLMENT_TOKEN --service" start= auto DisplayName= "Sentinel Agent"
) else (
    echo Service exists, updating configuration...
    sc config SentinelAgent binPath= "\"C:\Program Files\Sentinel Agent\sentinel-agent.exe\" --server=http://localhost:8081 --token=REDACTED_ENROLLMENT_TOKEN --service"
)
